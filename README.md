...
支持文本 + 图片 + 文件同步 (在一台电脑复制文件, 到另一台直接粘贴;
单次总大小上限 64MB, 暂不支持文件夹), 主窗口显示在线节点与同步记录,
关闭窗口后最小化到系统托盘继续后台运行。
...
- UDP 发现 (端口 48765): 周期性广播宣告自身, 并对已知/手动节点补发单播
  (应对路由器广播抑制); 心跳携带主机名和已知节点名单, 节点间互相介绍
  (gossip), 无需两两广播可达即可全部互联; 10 秒无心跳即视为下线
- 界面显示 主机名 (IP), 双击节点可设置别名; 托盘菜单可随时修改口令
